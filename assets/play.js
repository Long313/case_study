...
function gameOver(){
  ...
  canvas.innerHTML = '<audio autoplay src="./audio/Tieng-game-thua.mp3"></audio>';
...
function youWin(){
  ...
  canvas.innerHTML = '<audio autoplay src="./audio/Tieng-game-thang.mp3"></audio>';
  clearInterval(interval);
}
function play() {
  if (!isGameOver) {
    ...
    continueGame();
    // requestAnimationFrame(play); // như một hàm đệ quy, xử lý di chuyển mượt hơn so với dùng setInterval
  } else {
    gameOver();
    // alert("Chúc bạn may mắn lần sau !!!"); // điều kiện để dừng game.
  }
}
// play();
// chơi lại
